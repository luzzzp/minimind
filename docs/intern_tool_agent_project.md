...
```bash
cd scripts
python build_intern_tool_data.py --seed 42 --sft_size 5000 --dpo_size 2000 --rl_size 1000 --eval_size 200
```
...
DPO 负样本覆盖错误工具、错误参数、无工具硬答和格式异常。GRPO 数据保留可验证 `gt`，用于规则 Reward 和端到端评测。
...
- 工具名称是否在候选 tools 中。
- 参数是否满足工具 schema 的必要字段。
- 工具调用数量与可验证目标的匹配程度。
- 最终答案是否命中 `gt`。
- thinking 闭合、输出长度、重复 n-gram、未完成多轮调用惩罚。
...
```bash
cd trainer
python train_full_sft.py --from_weight pretrain --save_weight intern_sft --data_path ../dataset/intern_tool_sft.jsonl --epochs 2 --batch_size 24 --max_seq_len 1024 --save_interval 200

python train_dpo.py --from_weight intern_sft --save_weight intern_dpo --data_path ../dataset/intern_tool_dpo.jsonl --epochs 1 --batch_size 8 --max_seq_len 1024 --learning_rate 4e-8 --beta 0.15 --save_interval 100

python train_agent.py --from_weight intern_dpo --save_weight intern_grpo --data_path ../dataset/intern_tool_agent.jsonl --epochs 1 --batch_size 2 --num_generations 4 --max_seq_len 1024 --max_gen_len 384 --loss_type grpo --reward_mode rule --reward_model_path none --save_interval 20
```
...
```bash
cd scripts
python eval_intern_tool_agent.py --weight intern_sft --eval_path ../dataset/intern_tool_eval.jsonl
python eval_intern_tool_agent.py --weight intern_dpo --eval_path ../dataset/intern_tool_eval.jsonl
python eval_intern_tool_agent.py --weight intern_grpo --eval_path ../dataset/intern_tool_eval.jsonl
```
...
| intern_sft | 待实测 | 待实测 | 待实测 | 待实测 | 待实测 | 待实测 |
| intern_dpo | 待实测 | 待实测 | 待实测 | 待实测 | 待实测 | 待实测 |
| intern_grpo | 待实测 | 待实测 | 待实测 | 待实测 | 待实测 | 待实测 |
